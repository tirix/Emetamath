 ...
 * such as UTF-8.
 */
public class Messages implements MessageHandler {
    ...
    public boolean accumErrorMessage(final SourcePosition position, final String errorMessage, final Object... args) {
        return accumErrorMessage(errorMessage, args);
    }
    ...
    public boolean accumInfoMessage(final SourcePosition position, final String infoMessage, final Object... args) {
        return accumInfoMessage(infoMessage, args);
    }
    ...
    /**
     * @return true if errors have been counted
     */
    @Override
	public boolean hasErrors() {
		return errorMessageCnt > 0;
	}
    ...
    public String getOutputMessageText() {
        ...
        final StringBuilder sb = new StringBuilder(
            (getErrorMessageCnt() + getInfoMessageCnt())
                * 80); // guessing
                       // average
                       // message
                       // length
        String[] msgArray = getErrorMessageArray();
        int msgCount = getErrorMessageCnt();
        ...
        msgArray = getInfoMessageArray();
        msgCount = getInfoMessageCnt();
        for (int i = 0; i < msgCount; i++) {
            sb.append(msgArray[i]);
            sb.append("\n");
            sb.append(PaConstants.ERROR_TEXT_SPACER_LINE);
    ...
    public String getOutputMessageTextAbbrev() {
        if (getErrorMessageCnt() == 0 && getInfoMessageCnt() == 0)
            return null;
        final StringBuilder sb = new StringBuilder(
            (getErrorMessageCnt() + getInfoMessageCnt()) * 80);
        String[] errors = getErrorMessageArray();
        int i = getErrorMessageCnt();
        for (int j = 0; j < i; j++) {
            sb.append(errors[j]);
            sb.append('\n');
        }

        errors = getInfoMessageArray();
        i = getInfoMessageCnt();
        for (int k = 0; k < i; k++) {
            sb.append(errors[k]);
            sb.append('\n');
