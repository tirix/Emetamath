...
public class SeqAssigner implements TheoremLoaderCommitListener {
     ...
     * @param messageHandler the MessageHandler object for error logging
    ...
    public void rollback(final MMTTheoremSet mmtTheoremSet,
        final MessageHandler messageHandler, final boolean auditMessages)
    {
        ...
        for (final TheoremStmtGroup t : mmtTheoremSet) {
            ...
            for (int i = 0; i < wasLogHypInsertedArray.length; i++)
                if (wasLogHypInsertedArray[i]) {
                    if (auditMessages)
                        outputRollbackAuditMessage(messageHandler,
                            assignedLogHypSeq[i],
            ...
            if (wasTheoremInserted) {
                if (auditMessages)
                    outputRollbackAuditMessage(messageHandler, assignedTheoremSeq,
    ...
    private void outputRollbackAuditMessage(final MessageHandler messageHandler,
        final int seq, final String stmtCaption, final Stmt stmt,
        final String updateCaption)
    {
    	messageHandler.accumMessage(LangConstants.ERRMSG_SEQ_ASSIGNER_ROLLBACK_AUDIT,
