...
public class TheoremStmtGroup {
    ...
    public TheoremStmtGroup(final MMTTheoremFile mmtTheoremFile,
        final LogicalSystem logicalSystem, final MessageHandler messageHandler,
        final TlPreferences tlPreferences) throws TheoremLoaderException
    {
        ...
        validateStmtGroupData(logicalSystem, messageHandler, tlPreferences);
    ...
    public void updateLogicalSystem(final LogicalSystem logicalSystem,
        final MessageHandler messageHandler, final TlPreferences tlPreferences)
            throws TheoremLoaderException, LangException
    {
        if (getIsTheoremNew())
            addTheoremToLogicalSystem(logicalSystem, messageHandler, tlPreferences);
        else
            updateTheoremInLogicalSystem(logicalSystem, messageHandler,
                tlPreferences);
    ...
    private boolean isLabelInLogHypList(final String label) {
    	for (SrcStmt srcstmt : logHypSrcStmtList)
    		if (srcstmt.label.equals(label))
    			return true;
    	return false;
    }
     ...
     * @param messageHandler the error messages object
            ...
            throws TheoremLoaderException, LangException
    {
        ...
        if (tlPreferences.auditMessages.get())
        	messageHandler.accumMessage(TlConstants.ERRMSG_AUDIT_MSG_THEOREM_UPD,
            ...
            throws TheoremLoaderException, LangException
    {
        ...
        for (int i = 0; i < logHypArray.length; i++) {
            ...
            loadStmtParseTree(messageHandler, logicalSystem, logHypArray[i],
                currSrcStmt);
        }
        ...
        loadStmtParseTree(messageHandler, logicalSystem, theorem, theoremSrcStmt);
    ...
    private void loadStmtParseTree(final MessageHandler messageHandler,
        ...
        final SrcStmt currSrcStmt) throws TheoremLoaderException
    {
        ...
        final ParseTree parseTree = syntaxVerifier.parseOneStmt(messageHandler,
