...
public class MMTTheoremExportFormatter {
     ...
     * This version outputs only the theorem $p statement and its proof.
     *
     ...
     *             unified.
     */
    public List<StringBuilder> buildConclusionStringBuilderLineList(
     ...
     *
     * @param proofWorksheet ProofWorksheet object
     * @param proofStepOnly whether the full theorem with scope and varHyp shall 
     * 		   be output, or only the theorem $p statement and its proof.
     * @return LinkedList of StringBuilder objects each containing one line of
     *         text in Metamath-format (without newlines.)
     * @throws TheoremLoaderException if the ProofWorksheet is null or is not
     *             unified.
    ...
    private void outputProofLine(final ProofWorksheet proofWorksheet) {
        ...
        boolean compressedOutput = true; // TODO get this from the preferences?
        if(compressedOutput) {
        	outputCompressedProof(proofWorksheet, proofWorksheet.getQedStepProofRPN(), leftOffset, rightCol);
        } else {
        	outputProof(proofWorksheet.getQedStepProofRPN(), leftOffset, rightCol);
        }
    }

    private void outputCompressedProof(ProofWorksheet proofWorksheet, RPNStep[] proof, final int left,
            final int right) {
    	Stmt[] rpnProof = new Stmt[proof.length];
    	for(int i=0;i<proof.length;i++) rpnProof[i] = proof[i].stmt;
        CompressedProofStmt proofStmt = new CompressedProofStmt(proofWorksheet, rpnProof, left, right);
    	list.add(proofStmt.getStmtText());
    }
