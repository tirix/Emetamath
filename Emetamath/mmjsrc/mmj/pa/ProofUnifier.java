...
public class ProofUnifier {
    ...
    public boolean initializeLookupTables(final MessageHandler messageHandler) {
        ...
        if (!excl.isEmpty())
            messageHandler.accumMessage(PaConstants.ERRMSG_UNIFY_SEARCH_EXCLUDE,
                excl);
    ...
    public void unifyAllProofDerivationSteps(
        final ProofWorksheet proofWorksheet, final MessageHandler messageHandler,
    ...
    private void autoStepUnificationMethod() throws VerifyException {
            ...
            .entrySet())
        {
            ...
            if (proofAsstPreferences.djVarsSoftErrors
                ...
                messageHandler.accumException(addStepContext(
            ...
            .getProofWorkStmtList())
        {
                    ...
                    && derivStep.djVarsErrorStatus == DjVarsErrorStatus.Soft)
            {
                ...
                if (derivStep.alternateRefList != null) {
                    messageHandler.accumException(derivStep.heldDjErrorMessage);
    ...
    private void accumAlternateRefMessage() {
        if (!derivStep.alternateRefList.isEmpty())
            messageHandler.accumException(getException(
    ...
    private void attemptProofOfDerivStep() {
        ...
        if (proofAsstPreferences.recheckProofAsstUsingProofVerifier.get())
            if (!checkDerivStepProofUsingVerify()) {
                messageHandler.accumException(addStepContext(new ProofAsstException(
    ...
    private boolean checkDerivStepProofUsingVerify() {
        ...
        if (errmsg != null) {
            ...
            messageHandler.accumException(errmsg);
...
//          }
        ...
        final ParseNode[] checkUnificationAssrtSubst)
    {
        ...
        return addStepContext(new ProofAsstException(errmsg,
            PaConstants.ERRMSG_DV_VERIFY_ERR, checkUnificationRef));
    }
    ...
    private void markRefUnificationFailure(final Assrt assrt) {
        ...
        messageHandler.accumException(getException(PaConstants.ERRMSG_REF_UNIFY_ERR,
            derivStep.getRefLabel()).setPosition(derivStep.getPosition(PaConstants.FIELD_ID_REF)));
    ...
    public void reportUnificationFailures() {
        ...
        final Set<ProofStepStmt> usedSteps = new HashSet<>();

        for (final ProofWorkStmt s : proofWorksheet.proofWorkStmtList) {
            addUsedSteps(usedSteps, s);
            if (accessibleSteps.contains(s)
                && (derivStep = (DerivationStep)s).getRef() == null)
                messageHandler.accumException(
                    getException(PaConstants.ERRMSG_STEP_UNIFY_ERR)
                    .setPosition(derivStep.getPosition(PaConstants.FIELD_ID_REF)));
        }
       for (final ProofWorkStmt s : proofWorksheet.proofWorkStmtList)
            if (s instanceof DerivationStep && !usedSteps.contains(derivStep = (DerivationStep)s) && derivStep != proofWorksheet.qedStep)
                messageHandler.accumException(
                    getException(PaConstants.ERRMSG_STEP_UNUSED)
                    .setPosition(derivStep.getPosition(PaConstants.FIELD_ID_REF)));
    }

    private void addUsedSteps(final Set<ProofStepStmt> usedSteps,
            final ProofWorkStmt step)
    {
		if(!(step instanceof DerivationStep)) return;
        for (final ProofStepStmt hyp : ((DerivationStep)step).getHypList()) 
        {
            usedSteps.add((ProofStepStmt)hyp);
        	addUsedSteps(usedSteps, hyp);
        }
    }
        ...
        stepLoop: while (true) {
            ...
            if (selectorSearchStmt == derivStep) {
                if (proofWorksheet.stepRequest.type == StepRequestType.SelectorSearch)
                    proofWorksheet.stepSelectorStore = stepSelectorSearch
                        .loadStepSelectorStore(derivStep);
    ...
    public <T extends MMJException> T addStepContext(final T e) {
        return ProofWorksheet.addLabelContext(proofWorksheet, StepContext
            .addStepContext(derivStep == null ? null : derivStep, e));
    }
