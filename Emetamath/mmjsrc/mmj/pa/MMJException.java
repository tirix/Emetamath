...
import mmj.pa.PaConstants.StepContext;
...
public class MMJException extends Exception {
    ...
    public MMJException addContext(final ErrorContext ec) {
    	if (ec != null && getContext(ec.getClass()) == null)
            ctxt.add(ec);
    	if(position == null) determinePosition();
    	return this;
    }

    /**
     * Try to find out the position based on the added context.
     * Ideally, the SourcePosition object would always be passed when creating an exception.
     */
    protected void determinePosition() {
    	FileContext fileContext = null;
    	StepContext stepContext = null;
    	LineColumnContext lineColumnContext = null;
    	for(ErrorContext ec:ctxt) {
    		if(ec instanceof StepContext) stepContext = (StepContext)ec;
    		if(ec instanceof FileContext) fileContext = (FileContext)ec;
    		if(ec instanceof LineColumnContext) lineColumnContext = (LineColumnContext)ec;
    	}
    	if(stepContext != null && stepContext.step != null) position = stepContext.step.getPosition(PaConstants.FIELD_ID_REF);
    	if(fileContext == null || lineColumnContext == null) return;
    	position = new SourcePosition(fileContext.sourceId, lineColumnContext.lineNbr, lineColumnContext.columnNbr, lineColumnContext.charNbr, 1); // here the length is unknown
    }
    
    public MMJException setPosition(SourcePosition position) {
    ...
    public static class FormatContext implements ErrorContext {
		...
		public SourcePosition getPosition() {
			for(Object o:args)
				if(o instanceof SourceElement) return ((SourceElement)o).getPosition();
			return null;
		}
