...
public class HeaderStmt extends ProofWorkStmt {
    ...
    private boolean validateTheoremLabel() {
        ...
        if (stmt == null) {
            if (!Statementizer.areLabelCharsValid(theoremLabel)) {
                w.messageHandler.accumMessage(
                	w.proofTextTokenizer.getCurrentPosition(),
                	PaConstants.ERRMSG_BAD_LABEL_CHAR,
                	theoremLabel);
                headerInvalid = true;
                return false;
            }
            if (Statementizer.isLabelOnProhibitedList(theoremLabel)) {
                w.messageHandler.accumMessage(
                	w.proofTextTokenizer.getCurrentPosition(),
                    PaConstants.ERRMSG_PROHIB_LABEL,
                    theoremLabel);
                headerInvalid = true;
                return false;
            }
            if (w.logicalSystem.getSymTbl().containsKey(theoremLabel)) {
                w.messageHandler.accumMessage(
                	w.proofTextTokenizer.getCurrentPosition(),
                    PaConstants.ERRMSG_STMT_LABEL_DUP_OF_SYM_ID, 
                    theoremLabel);
                headerInvalid = true;
                return false;
            }
        ...
        w.messageHandler.accumMessage(w.proofTextTokenizer.getCurrentPosition(), PaConstants.ERRMSG_NOT_A_THRM, theoremLabel);
        headerInvalid = true;
        return false;
    }
    ...
    private boolean validateLocAfterLabel() {
        ...
        if (stmt == null) {
            w.messageHandler.accumMessage(w.proofTextTokenizer.getCurrentPosition(),
                PaConstants.ERRMSG_LOC_NOTFND,
