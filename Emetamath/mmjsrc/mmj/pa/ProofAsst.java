...
public class ProofAsst implements TheoremLoaderCommitListener {
     ...
     *
     * @param messageHandler the mmj.lang.MessageHandler object used to store error and
     *            informational messages.
     * @return initializedOK flag.
     */
    public boolean initializeLookupTables(final MessageHandler messageHandler) {
        this.messageHandler = messageHandler;
        initializedOK = proofUnifier.initializeLookupTables(messageHandler);
        proofAsstPreferences.getSearchMgr().initOtherEnvAreas(this,
            logicalSystem, grammar, verifyProofs, messageHandler);
     ...
     */
    public MessageHandler verifyAllProofs() {
     ...
     * @return MessageHandler object.
     */
    public MessageHandler loadTheoremsFromMMTFolder() {
        try {
            theoremLoader.loadTheoremsFromMMTFolder(logicalSystem, messageHandler);
     ...
     * @return MessageHandler object.
     */
    public MessageHandler exportViaGMFF(final String proofText) {

        try {
            for (final GMFFException confirm : logicalSystem.getGMFFManager()
                .exportProofWorksheet(proofText, null))
                messageHandler.accumException(confirm);
        } catch (final GMFFException e) {
            messageHandler.accumException(new ProofAsstException(e,
     ...
     * @return MessageHandler object.
     */
    public MessageHandler extractTheoremToMMTFolder(final Theorem theorem) {
        try {
    ...
    public ProofWorksheet startNewNextProof(final int currProofMaxSeq) {
        ...
        if (theorem == null) {
            messageHandler.accumMessage(PaConstants.ERRMSG_PA_FWD_BACK_SEARCH_NOTFND,
                "forward");

            w = new ProofWorksheet(proofAsstPreferences, messageHandler, // oh yeah,
                                                                   // we got 'em
            ...
            w = new ProofWorksheet(theorem.getLabel(), proofAsstPreferences,
                logicalSystem, grammar, messageHandler);
        ...
        final boolean exportFormatUnified, final HypsOrder hypsOrder)
    {
        ...
        if (theorem == null) {
            ...
            w = new ProofWorksheet(proofAsstPreferences,
                /* oh yeah, we got 'em */messageHandler, /* structuralErrors=*/true,
                cursor);
        }
        else {
    ...
    public ProofWorksheet unify(final boolean renumReq,
    	...
    	return unify(renumReq, noConvertWV, new StringSource(proofText, "Proof Text"), preprocessRequest, stepRequest, tlRequest, inputCursorPos, printOkMessages);
    }
   ...
   /**
     ...
     * @param proofText proof text from ProofAsstGUI screen, or any String
     *            conforming to the formatting rules of ProofAsst.
     * @param source an identifier of the source being unified
     * @param renumReq renumbering of proof steps requested
     * @param noConvertWV true if we should not replace work vars with dummy
     *            vars in derivation steps
     * @param preprocessRequest if not null specifies an editing operation to be
     ...
     */
    public ProofWorksheet unify(final boolean renumReq,
        final boolean noConvertWV, Source source,
        final PreprocessRequest preprocessRequest,
        final StepRequest stepRequest, final TLRequest tlRequest,
        final int inputCursorPos, final boolean printOkMessages)
    {
        ...
        if (preprocessRequest == null)
            proofTextEdited = null;
        else
            try {
                proofTextEdited = preprocessRequest.doIt(source.getContents());
                source.replace(0, source.getSize(), proofTextEdited); // TODO - pass the source, and let the preprocessor do the changes there
                ...
                messageHandler.accumException(new MMIOException(e, MMIOConstants.ERRMSG_LOAD_MISC_IO, source.toString(), e.getMessage()));
                return updateWorksheetWithException(null, null);
            }

        final boolean[] errorFound = new boolean[1];
        final ProofWorksheet proofWorksheet = getParsedProofWorksheet(
	            source, errorFound, inputCursorPos, stepRequest);
        if (!errorFound[0]) {
    ...
    public ProofWorksheet tmffReformat(final boolean inputCursorStep,
    	...
    	return tmffReformat(inputCursorStep, proofText, "Proof Text", inputCursorPos);
    }
     ...
     */
    public ProofWorksheet tmffReformat(final boolean inputCursorStep,
        final String proofText, final String sourceName, final int inputCursorPos)
    {

        final boolean[] errorFound = new boolean[1];

        final ProofWorksheet proofWorksheet = getParsedProofWorksheet(
        	new StringSource(proofText, sourceName), 
        ...
        final boolean debugOutput, final boolean supportPrefix)
    {
        final TransformationManager trManager = enabled
            ? new TransformationManager(this, getSortedAssrtSearchList(),
                getProvableLogicStmtTyp(), messageHandler, verifyProofs,
        ...
        final int cursorPos, final int selectionNumber)
    {
                ...
                macroManager))
        {
            ...
            if (proofWorksheet.stepSelectorStore == null) {
                messages.accumException(addLabelContext(proofWorksheet,
                    PaConstants.ERRMSG_STEP_SELECTOR_BATCH_TEST_NO_RESULTS));
                return;
            }

            final StepSelectorResults results = proofWorksheet.stepSelectorStore.getResults();
     ...
     *
     * @param sourceId provider of proof text from ProofAsstGUI screen, or any 
     *            String conforming to the formatting rules of ProofAsst.
     ...
     * @return ProofWorksheet unified.
     */
    private ProofWorksheet getParsedProofWorksheet(final Source source, 
        ...
        final StepRequest stepRequest)
    {
        ...
        try (
            ProofWorksheetParser proofWorksheetParser = new ProofWorksheetParser(
                source, proofAsstPreferences, logicalSystem,
                grammar, messageHandler, macroManager))
        {
            ...
            messageHandler.accumException(addLabelContext(proofWorksheet, e));
            proofWorksheet = updateWorksheetWithException(proofWorksheet, e);
        } catch (final MMIOException e) {
            messageHandler.accumException(addLabelContext(proofWorksheet, e));
        } catch (final IOException e) {
            e.printStackTrace();
            messageHandler.accumException(
                addLabelContext(proofWorksheet, new ProofAsstException(e,
    ...
    {
        messageHandler.accumMessage(PaConstants.ERRMSG_PA_TESTMSG_03,
        ...
        if (timeTop != null) {
            messageHandler.accumMessage(PaConstants.ERRMSG_PA_TIME_TOP_HEADER);
        ...
        final VolumeTestStats stats, final boolean printOkTheorems)
    {
        ...
        if (q == null) {
            ...
            messageHandler.accumMessage(PaConstants.ERRMSG_PA_TESTMSG_01,
        ...
        if (q != null) {
            ...
            if (differenceFound && printOkTheorems) {
                if (stats != null)
                    stats.nbrTestProvedDifferently++;
                messageHandler.accumMessage(PaConstants.ERRMSG_PA_TESTMSG_02,
        ...
        final boolean noConvertWV, final boolean printOkMessages)
    {
                ...
                && proofWorksheet.stepRequest.type == StepRequestType.GeneralSearch)
        {
            ...
            if (rpnProof == null)
                ...
                if (printOkMessages)
                    messageHandler.accumException(addLabelContext(proofWorksheet,
        ...
        final MMJException e)
    {
        ...
        final ProofAsstCursor proofCursor = c == null ? new ProofAsstCursor()
            : new ProofAsstCursor((int)c.charNbr, (int)c.lineNbr,
                (int)c.columnNbr);
        if (w == null)
            out = new ProofWorksheet(proofAsstPreferences, messageHandler, true, // structural
                                                                           // err!
                proofCursor);
        else {
            out = w;
