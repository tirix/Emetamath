...
public class SearchOutput {
    ...
    public void finalize(final CompiledSearchArgs csa,
        final BookManager bookManager, final MessageHandler messageHandler)
    {
        endTimeMillis = System.currentTimeMillis();
        elapsedMillis = endTimeMillis - startTimeMillis;
        if (csa.searchStats > 0)
            dumpStats(csa, bookManager, messageHandler);
    }
        ...
        final BookManager bookManager, final MessageHandler messageHandler)
    {
        if (args.searchStats <= 0)
            return;
        messageHandler.accumInfoMessage("SearchOutput.dumpStats(): " + searchTitle);
        if (args.searchStats >= 5) {
            dumpChapSecHierarchy(args, bookManager, messageHandler);
            return;
        }
        if (args.searchStats >= 4) {
            dumpSearchResults(args, bookManager, messageHandler);
            return;
        }
        if (args.searchStats >= 3) {
            dumpSearchArgs(args, bookManager, messageHandler);
            return;
        }
        if (args.searchStats >= 2) {
            dumpDetailedStats(args, bookManager, messageHandler);
            return;
        }
        if (args.searchStats >= 1) {
            dumpSummaryStats(args, bookManager, messageHandler);
            return;
        }
        return;
    }
    ...
    public void dumpSummaryStats(final CompiledSearchArgs csa,
        final BookManager bookManager, final MessageHandler m)
    {
