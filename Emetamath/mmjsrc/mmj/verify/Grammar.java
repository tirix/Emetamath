...
//  }
     ...
     */
    public ParseTree parseOneStmt(final MessageHandler messageHandler,
        final Map<String, Sym> symTbl, final Map<String, Stmt> stmtTbl,
        final Stmt stmt)
    {

        checkVerifySyntaxParams(messageHandler, symTbl, stmtTbl);
        ...
        return grammaticalParseOneFormula(stmt.getFormula(),
            stmt.getMandVarHypArray(), stmt.getSeq(), stmt.getLabel(), stmt.getPosition());
     ...
     */
    public ParseTree parseFormula(final MessageHandler messageHandler,
        final Map<String, Sym> symTbl, final Map<String, Stmt> stmtTbl,
        ...
        final Stmt defaultStmt)
    {
        ...
        return grammaticalParseOneFormula(formula, varHypArray, highestSeq,
            defaultStmt == null ? null : defaultStmt.getLabel(), null); // TODO provide position
    }
     ...
     */
    public void parseAllFormulas(final MessageHandler messageHandler,
        final Map<String, Sym> symTblParam,
        final Map<String, Stmt> stmtTblParam,
        final Progress progress)
    {
        checkVerifySyntaxParams(messageHandler, symTblParam, stmtTblParam);
        if (!grammarInitialized) {
            try {
                initializeGrammarTables();
            } catch (final VerifyException e) {
        ...
        if(progress != null) progress.addTask(stmtTblBySeq.size()/1000+1);
        int counter = 0;
        for (final Stmt stmt : stmtTblBySeq) {
        	if(progress != null && counter++ % 1000 == 0) progress.worked(1);
            ...
            ParseTree exprParseTree = grammaticalParseOneFormula(
                stmt.getFormula(), varHypArray, stmt.getSeq(), stmt.getLabel(), stmt.getPosition());
     ...
     */
    public boolean initializeGrammar(final MessageHandler messageHandler,
        final Map<String, Sym> symTblParam,
        final Map<String, Stmt> stmtTblParam)
    {
        grammarInitialized = false;
...
//  private Stmt[] grammaticalParseOneFormula(
    private ParseTree grammaticalParseOneFormula(final Formula formula,
        ...
        final String defaultStmtLabel, final SourcePosition position)
    {
//      Stmt[]      exprRPN = null;
        ParseTree exprParseTree = null;
...
//      return exprRPN;
    ...
    private void checkVerifySyntaxParams(final MessageHandler messageHandlerParam,
        final Map<String, Sym> symTblParam,
        final Map<String, Stmt> stmtTblParam)
    {
...
//      axiom.setExprRPN(syntaxRPN);
        ...
        if (mandFrame.djVarsArray.length != 0) {
        	messageHandler.accumMessage(
        		axiom.getPosition(),
        		GrammarConstants.ERRMSG_DJ_VARS_ON_SYNTAX,
                axiom.getLabel());
            errorsFound = true;
        }
        if (mandFrame.hypArray.length != varHypArray.length) {
        	messageHandler.accumMessage(
        		axiom.getPosition(),
        		GrammarConstants.ERRMSG_LOGHYP_FOR_SYNTAX,
                axiom.getLabel());
            errorsFound = true;
        }
