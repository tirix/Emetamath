...
public class VerifyProofs implements ProofVerifier {
    ...
    private final static int STEP = 2; // steps with which progress is reported
    ...
    private Progress verifyProgress;
     ...
     */
    public void verifyAllProofs(final MessageHandler messageHandler,
        final Map<String, Stmt> stmtTbl)
    {
        verifyProgress.addTask(stmtTbl.size());
        int lasti = 0, i = 0, step = stmtTbl.size() * STEP / 100;
        ...
        for (final Stmt stmt : list) {
            ...
            if (stmt instanceof Theorem) {
                ...
                if(i - lasti > step) { verifyProgress.worked(i - lasti); lasti = i; }
                if (errMsg != null)
...
//                ProofConstants.ERRMSG_PROOF_STACK_UNDERFLOW);
    ...
    {
        throw TheoremContext.addTheoremContext(proofStmtLabel, 
        	StepContext.addStepContext(stepLabel,
        	StepContext.addSourcePositionContext(proofStmt.getPosition(),
        	new VerifyException(code, args))));
        	}
	...
	public void setVerifyProgress(Progress progress) {
